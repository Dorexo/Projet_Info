<?php
    ...
    function dbGetPlaylists($db,$id_user){
        try{
            $request = "SELECT p.id_playlist, p.nom, al.image, p.date_creation, (SELECT count(*) from musique_dans_playlists mdp  WHERE mdp.id_playlist=p.id_playlist) FROM playlists p LEFT JOIN musique_dans_playlists c ON c.id_playlist=p.id_playlist LEFT JOIN  musiques m ON m.id_musique=c.id_musique LEFT JOIN albums al ON m.id_album=al.id_album and m.id_musique=(SELECT id_musique FROM musique_dans_playlists l WHERE p.id_playlist=l.id_playlist LIMIT 1) where p.id_user=:id_user and p.nom!='Favoris' and p.nom!='Historique' ORDER BY UPPER(p.nom)";
            $statement = $db->prepare($request);
    ...
    function dbGetLastMusique($db,$id_user){
        try {
            $request = "SELECT m.id_musique FROM musiques m JOIN musique_dans_playlists mdp ON m.id_musique=mdp.id_musique JOIN playlists p ON p.id_playlist=mdp.id_playlist WHERE id_user = :id_user and p.nom='Historique' ORDER BY mdp.date_ajout DESC LIMIT 2";
            $statement = $db->prepare($request);
            $statement->bindParam(':id_user', $id_user);    
            $statement->execute();            
            return $statement->fetchall(PDO::FETCH_ASSOC)[1]['id_musique'];
        } catch (PDOException $exception){
            error_log('Request error: '. $exception->getMessage());
            return false;
        }
